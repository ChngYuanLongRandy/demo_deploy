from src.config.config import TOTAL_FEATURES, PIPELINE_PATH
...
def test_predict():
  ...
  pred_proba_processed_sample_input_0 = make_prediction_inputs(processed_sample_input_0, proba= True)
  pred_proba_processed_sample_input_1 = make_prediction_inputs(processed_sample_input_1, proba= True)
  pred_proba_processed_sample_input_2 = make_prediction_inputs(processed_sample_input_2, proba= True)

  # survival rate for sample input 0 should be higher than sample input 1
  assert(pred_proba_processed_sample_input_0[0][1] > pred_proba_processed_sample_input_1[0][1])

  # survival rate for sample input 0 should be higher than sample input 2
  assert(pred_proba_processed_sample_input_0[0][1] > pred_proba_processed_sample_input_2[0][1])